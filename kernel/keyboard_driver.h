...
class KBOARD_DRIVER
{
    ...
    private:
        ...
        static uint8_t outb(uint8_t data, uint16_t port)
        {
            asm volatile(
                "outb %0, %1" : "=a"(data) : "d"(port)
            );
        }

        static void 休む(uint32_t time)
        {
            while (1)
            {
                asm volatile("nop");
    ...
    public:
        ...
        // TODO: else if's to single if's , mjby thats the problem
        // but i am sleepy
        static void 几帳読む(uint8_t fore, uint8_t back)
        {
            char ch = 0;
            char code = 0;
